...
*   If open, close SasView. Then go to your profile folder (eg, on Windows: C:\\Users\\user\_name) and open the .sasview folder. Delete the file categories.json. Run SasView again.
...
5.  Note that the value (0 or 1) of the REG\_DWORD called 'ProxyEnable' appears to be irrelevant!
6.  Close the Registry Editor.
7.  Click on the Start button. In the 'search programs and files' box type 'environment variables' and press {return}.
8.  In the Environment Variables panel, check the _User_ variables (not System variables) for HTTP\_PROXY and HTTPS\_PROXY. If they exist, ensure that both specify your username, proxy server and port. If they are missing, create these variables as follows:

*   Click 'New'. Enter HTTP\_PROXY as the 'Variable name'. Enter a 'Variable value' of the form: username@proxy.uni.edu:port. Click 'Ok'. Repeat the process to create the variable HTTPS\_PROXY. Click 'Ok'.
